...
if($_POST){
    $res = $db->prepare("UPDATE citizen1 SET nomi=?,surname=?,born_year=?,passport=?,email=?,kasb_id=?,jinoyat_id=?,mukofot_id=? WHERE id=?")
              ->execute(array($_POST["ism"],$_POST["familiya"],$_POST["tug_sana"],$_POST["pass_seriya"],$_POST["email"],$_POST["kasb_id"],$_POST["jinoyat_id"],$_POST["mukofot_id"],$id));
    if ($res !== false){
